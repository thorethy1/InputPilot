...
void BLEOTA::notify(const char*event,const char*error){if(!status_)return;char json[900];if(strcmp(event,"IDLE")==0)snprintf(json,sizeof(json),"{\"product\":\"%s\",\"board\":\"%s\",\"deviceId\":\"%s\",\"deviceName\":\"%s\",\"protocol\":%u,\"otaSchema\":%u,\"firmware\":\"%s\",\"authRequired\":%s,\"capabilities\":[\"ble_control\",\"wifi_control\",\"keep_awake_v2\",\"pairing_input_test\",\"ble_ota\",\"wifi_ota\",\"ble_diagnostics\",\"wifi_diagnostics\",\"usb_identity\",\"mouse_move\",\"mouse_click\",\"mouse_button_state\",\"mouse_scroll\",\"keyboard_type\",\"keyboard_key\",\"keyboard_layout\",\"release_all\",\"protocol_v1\"],\"state\":\"idle\",\"event\":\"IDLE\",\"offset\":0,\"size\":0,\"maxChunk\":500,\"windowSize\":%lu}",FW_PRODUCT,FW_BOARD,DeviceIdentity::deviceId(),BLE_DEVICE_NAME,OTA_PROTOCOL_VERSION,OTA_SCHEMA_VERSION,FW_VERSION,strlen(CONTROL_API_TOKEN)?"true":"false",(unsigned long)BLE_OTA_ACK_BYTES);else snprintf(json,sizeof(json),"{\"protocol\":%u,\"otaSchema\":%u,\"firmware\":\"%s\",\"state\":\"%s\",\"event\":\"%s\",\"offset\":%lu,\"size\":%lu,\"maxChunk\":500,\"windowSize\":%lu%s%s%s}",OTA_PROTOCOL_VERSION,OTA_SCHEMA_VERSION,FW_VERSION,OTAProtocol::stateName(g_otaEngine.state()),event,(unsigned long)g_otaEngine.received(),(unsigned long)g_otaEngine.total(),(unsigned long)BLE_OTA_ACK_BYTES,error?",\"error\":\"":"",error?error:"",error?"\"":"");status_->setValue((const uint8_t*)json,strlen(json));status_->notify();}
