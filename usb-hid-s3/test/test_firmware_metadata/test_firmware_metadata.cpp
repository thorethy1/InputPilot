...
void test_parses_inputpilot_metadata_inside_image() {
  ...
  image.push_back('\0');
  image += "noise" FW_METADATA_PREFIX "product=InputPilot;board=esp32-s3-zero-4mb;version=0.8.11;protocol=2;otaSchema=1;";
  auto value = parse(image);
  TEST_ASSERT_EQUAL_STRING("InputPilot", value.product.c_str());
  TEST_ASSERT_EQUAL_STRING("0.8.11", value.version.c_str());
}
...
void test_rejects_wrong_board_and_protocol() {
  auto board = parse(FW_METADATA_PREFIX "product=InputPilot;board=other;version=0.8.11;protocol=2;otaSchema=1;");
  std::string error;
  TEST_ASSERT_FALSE(FirmwareMetadata::compatible(board, error));
  TEST_ASSERT_EQUAL_STRING("incompatible_board", error.c_str());
  auto protocol = parse(FW_METADATA_PREFIX "product=InputPilot;board=esp32-s3-zero-4mb;version=0.8.11;protocol=3;otaSchema=1;");
